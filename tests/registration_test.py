...
class TestRegistration():
    ...
    def test_registration_of_the_user(self, driver, fake_email, fake_password):
        WebDriverWait(driver, 3).until(
            expected_conditions.visibility_of_element_located(MainPageLocators.ENTRY_AND_REGISTRATION_BUTTON))
        RegistrationOfAccount.register_account(driver, fake_email, fake_password)
        WebDriverWait(driver, 3).until(
    ...
    def test_user_registration_email_differs_from_the_mask(self, driver, incorrect_fake_email):
        WebDriverWait(driver, 3).until(
            expected_conditions.visibility_of_element_located(MainPageLocators.ENTRY_AND_REGISTRATION_BUTTON))
        RegistrationOfAccount.an_account_with_an_incorrect_email(driver, incorrect_fake_email)
        ...
        assert driver.find_element(*LoginAndRegistrationLocators.EMAIL_ALREADY_EXISTS_ERROR_TEXT).is_displayed()
        assert driver.find_element(*LoginAndRegistrationLocators.EMAIL_HIGHLIGHTED_IN_RED).is_displayed()
        assert driver.find_element(*LoginAndRegistrationLocators.PASSWORD_HIGHLIGHTED_IN_RED).is_displayed()
        assert driver.find_element(*LoginAndRegistrationLocators.REPEAT_PASSWORD_HIGHLIGHTED_IN_RED).is_displayed()

    def test_registration_existing_user(self, driver, fake_email, fake_password):
        WebDriverWait(driver, 3).until(
            expected_conditions.visibility_of_element_located(MainPageLocators.ENTRY_AND_REGISTRATION_BUTTON))
